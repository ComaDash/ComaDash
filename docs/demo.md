...
- `COMASA - Maestro de operación, mantenimiento y costos`: full jury overview.
- `COMASA - Vista operador`: live variables, alarms, and immediate action.
- `COMASA - Vista mantenimiento predictivo`: asset risk, MTBF/MTTR, and maintenance backlog.
- `COMASA - Vista jefatura y finanzas`: OEE, energy, biomass, water, vapor, condensate, downtime, and cost impact.
...
4. If screenshots/video are missing, open the dashboard JSON files under `grafana/dashboards/` and describe what each role would see: raw telemetry, `anomaly_event`, `maintenance_recommendation`, and `kpi` measurements.
